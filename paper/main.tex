...
We test this prediction using FineWeb \citep{penedo2024fineweb} and FineWeb-2 \citep{penedo2025fineweb2}, which ship source URLs for every document. We sample 350 domains per language in two stages, fetch each live robots.txt, and parse fifteen AI crawlers.\footnote{We release the 4,900 archived robots.txt bodies, their parses, and the analysis code at \url{https://anonymous.4open.science/r/consent-audit-release-CFBC}.} Domain-level rates are primary; composition estimates carry a weaker Horvitz--Thompson correction. The question is whether the gradient is in naming an AI agent, survives size and site-type controls, and changes the composition a consent-respecting rebuild would inherit.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.55\textwidth]{fig_gradient_v2.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_estimands.pdf}
